    ...
    const req = https.request({
      ...
      res.on('end', () => {
        ...
        try {
          ...
          if (parsed.file) {
            // Use CDN URL with no transformations — must be publicly accessible
...
async function renderWithShotstack(plan, mediaUrls, shotstackKey) {
  ...
  const watermark = {
    asset: { type: 'title', text: 'firstsample.co', style: 'minimal', color: '#333333', size: 'x-small', background: 'none', position: 'bottom' },
    start: 0,
    length: timeAccum,
  };
  ...
  console.log('Shotstack payload:', JSON.stringify(payload, null, 2).substring(0, 1000));
  return new Promise((resolve, reject) => {
