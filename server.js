...
async function uploadToCloudinary(filePath, filename) {
  ...
  const publicId = `firstsample/${uuidv4()}`;
    ...
    const req = https.request({
      ...
      res.on('end', () => {
        try {
          ...
          else reject(new Error('Cloudinary error: ' + JSON.stringify(parsed)));
        } catch (e) { reject(e); }
      });
    });
    req.on('error', reject);
...
// ── SHOTSTACK RENDER ──
async function renderWithShotstack(plan, cloudinaryUrls, shotstackKey) {
  let timeAccum = 0;

  const clips = plan.clips.map((clip, i) => {
    const src = cloudinaryUrls[i] || 'https://shotstack-assets.s3.amazonaws.com/footage/black.mp4';
    const isVideo = src.match(/\.(mp4|mov|webm|avi)/i) || src.includes('/video/');
      ...
      const req = https.request({
        ...
        res.on('end', () => {
          try {
            ...
            else if (status === 'failed') reject(new Error('Shotstack render failed'));
...
app.post('/api/generate', upload.array('files', 20), async (req, res) => {
  ...
  try {
    ...
    console.log(`[${jobId}] Uploading ${req.files?.length || 0} files to Cloudinary...`);
    ...
    const renderId = await renderWithShotstack(plan, cloudinaryUrls, shotstackKey);
